...
# Import ATAC data
summary_csv <- fread("../../../pearson_mtscatac_large_data_files/input/pbmcs_scatac/P3H2/outs/singlecell.csv") %>%
...
# Set it up
mat <- Read10X_h5("../../../pearson_mtscatac_large_data_files/input/pbmcs_scatac/P3H2/outs/filtered_peak_bc_matrix.h5")
fragments_file <- "../../../pearson_mtscatac_large_data_files/input/pbmcs_scatac/P3H2/outs/fragments.tsv.gz"
...
FeaturePlot(pbmc2,  features= c("sh2"), cols = viridis(100), 
            split.by = "Patient")
...
# Pearson correlation to see factors most assocaited with activation / not
...
FindMarkers(pbmc2, ident.1 = "6", ident.2 = "2")
...
pbmc2 <- Footprint(
  object = pbmc2,
  motif.name = c("MA0099.3", "MA1137.1"),
  genome = BSgenome.Hsapiens.UCSC.hg19,
  in.peaks = TRUE
)
Idents(pbmc2) <- "Patient"
pbmcT <- subset(pbmc2, seurat_clusters %in% c(0,1,2,4,6,7,8,9,10))
PlotFootprint(pbmcT, features = c("MA1137.1"))
