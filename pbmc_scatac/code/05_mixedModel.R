...
rv <- rowVars(gs.mat3)
variable_genes <- rv[rv >sort(rv, decreasing = TRUE)[100]]

lapply(rownames(gs.mat3), function(gene){
  
  result = tryCatch({
    df_gene <- data.frame(
      gene = as.numeric(gs.mat3[gene,]),
      patient = df_filt$patient,
      celltype = df_filt$predicted.id,
      heteroplasmy = df_filt$heteroplasmy / 100
    )
    mod <- lmer(data = df_gene, gene ~ heteroplasmy + (1|celltype) + (1|patient))
...
ggplot(results_df, aes(x = Estimate, y = -log10(pvalue))) +
  geom_point()
results_df %>% filter(pvalue < 10e-10)
