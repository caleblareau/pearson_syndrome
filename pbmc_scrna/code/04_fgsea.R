...
# Import genes
diff_df <- readRDS("../output/5March-PearsonRNAseq-diffGE-edgeR.rds")
ct <- "Activated B-cells"
...
enrich_out_all %>% group_by(celltype) %>% summarize(n = sum(padj < 0.01))

# Filter such that a pwayway has to be significant in at least 1 tissue
pathways_sig <- enrich_out %>% filter(padj < 0.01 ) %>% pull(pathway) %>% unique
