...
p_a <- ggplot(ddf, aes(x = donor, y = celltype, fill = log2(CD52 + 1))) +
  geom_tile() +
  scale_fill_gradientn(colors = jdb_palette("solar_rojos"), limits = c(10,16), oob = scales::squish) +
...
p_a
cowplot::ggsave2(p_a, file = "../plots/CD52_expression.pdf", width = 2, height = 2.3)

p_a <- ggplot(ddf, aes(x = donor, y = celltype, fill = log2(ACTB + 1))) +
  geom_tile() +
  scale_fill_gradientn(colors = jdb_palette("solar_rojos"), limits = c(10,16), oob = scales::squish) +
  scale_x_discrete(expand = c(0,0)) +
  scale_y_discrete(limits = rev(levels(as.factor(ddf$celltype))), expand = c(0,0)) +
  ...
  pretty_plot(fontsize = 8) + L_border() + theme(legend.position = "none") 
p_a
cowplot::ggsave2(p_a, file = "../plots/ACTB_expression.pdf", width = 2, height = 2.3)
