...
lapply(unique(diff_df$celltype), function(ct_go){
  ...
  geneList <- ct[["logFC"]]*-1; names(geneList) <- ct[["gene_id"]]; geneList <- sort(geneList, decreasing = TRUE)
...
names(d_con) <- c("Cytochrome bc1 complex", "Cytochrome c oxidase", "F-type ATPase, eukaryotes", 
           "NADH dehydrogenase (ubiquinone) 1 beta subcomplex", "Glycolysis (Embden-Meyerhof pathway), glucose => pyruvate",
...
p2 <- ggplot(pathways_sig, aes(x = log_padj, y = d_easy, fill = SZ > 0)) +
  ...
  theme(legend.position = "bottom") + scale_x_continuous(expand = c(0,0), limits = c(0,15)) +
  theme(axis.text.y = element_blank(),
        axis.ticks.y = element_blank())

cowplot::ggsave2(cowplot::plot_grid(p1, p2, nrow = 1, rel_widths = c(2.5, 1)), filename = "../plots/KEGG_enrichment.pdf", width = 7.5, height = 3)
