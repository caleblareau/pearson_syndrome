...
ps <- gsub("_del_find.SA_coverage_viz.pdf", "", list.files("namdc-data//", pattern = "*.SA_coverage_viz.pdf"))

lapply(ps, function(pid){
  fread(paste0("namdc-data/",pid,"_del_find.clip.tsv")) %>% 
    filter(SA > 5) %>% mutate(pid)
}) %>% rbindlist() -> clip_df 
...
clip_df %>% group_by(position) %>%
  ...
  summarize(n = n(), sumSA = sum(SA), sumClip = sum(clip_count), pid = pid) %>% filter(n == 1) %>% arrange(desc(sumSA)) %>%
  group_by(pid) -> top_clips


lapply(ps, function(pid){
  fread(paste0("namdc-data//",pid,"_del_find.SA.tsv")) %>%
    mutate(c1 = pmin(out1, out2), c2 = pmax(out1, out2)) %>%
    group_by(c1, c2) %>%
    summarize(count = n()) %>% mutate(donor = pid)
}) %>% rbindlist() -> SA_df 
...
cowplot::ggsave2(p1, filename = "plots/NAMDC_junctions.pdf", width = 2.8, height = 1)
