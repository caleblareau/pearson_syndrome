...
p1 <- ggplot(cdf %>% filter(coverage > 5), aes(x = coverage, color = what)) + 
...
p2 <- ggplot(cdf %>% filter(clip_coverage > 5), aes(x = clip_coverage, color = what)) + 
  stat_ecdf() + scale_x_log10() + pretty_plot(fontsize = 8) + L_border() +
  scale_color_manual(values = c("dodgerblue3", "firebrick")) +
