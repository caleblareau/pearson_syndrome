...
fig2 <- merged %>% dplyr::filter(del %in% c("13157_15477","9232_13413","8482_13445"))
merged <-fig2 %>%
  mutate(
...
fig1 <- merged %>% dplyr::filter(del %in% c("6073_13095", "8482_13447", "10381_15407"))
merged <-fig1 %>%
  mutate(
    b_l = matrix(unlist(strsplit(cell_id,'_')), ncol=5,byrow=T)[,4],
    info = matrix(unlist(strsplit(cell_id,'_')), ncol=5,byrow=T)[,5],
...
cowplot::ggsave2(p1, file = "../output/RMSE_pearson_del_primary.pdf", width = 5.7, height = 2.7)


# Do it again for figure 2
