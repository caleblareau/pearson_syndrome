...
cowplot::ggsave2(p1, file = "../output/precision_sim_1percentcutoff.pdf", width = 1.7, height = 1.7)

#----------------------
# Simulate sensitivity
#----------------------
conditions_df_sensitivity <- data.frame(
  ndelreads = round(c(300,500, 700, 900, 1100, 1300, 1500, 2000)*4/10),
  nwtreads = c(300,500, 700, 900, 1100, 1300, 1500, 2000)*4,
  ndelreads2 = c(300,500, 700, 900, 1100, 1300, 1500, 2000)*2
)[1:6,] # drop the last 2 because exceeds 50x and w/e
rrd_sens <- rbind(
...
cowplot::ggsave2(plot_sensistivity, file = "../output/sensitivity_plot05.pdf", width = 1.7, height = 1.7)
...
cowplot::ggsave2(px, file = "../output/error_heteroplsamy.pdf", width = 3.5, height = 1.8)
