...
clip_file <- "../data/pearson_mix_PD1.clip.tsv"
SA_file <- "../data/pearson_mix_PD1.SA.tsv"
...
p3 = ggplot(df3, aes(x = pos, y = clipped)) +
  geom_col(aes(fill = as.factor(color)), width = 50,
           show.legend = FALSE) +
  scale_fill_manual(values=c("lightgrey", "black", "firebrick")) +
  geom_label_repel(
    data = (df3 %>% arrange(-SA))[1:4,],
    aes(label = pos, color = as.factor(color)),
    size = 2,
    show.legend = FALSE) +
  scale_color_manual(values=c("lightgrey", "black", "firebrick")) +
  theme_bw() +
  xlab("position") +
  ylab("# of clipped reads") +
  geom_curve(aes(x = position, y = 0, xend = supp_A, yend = 0, alpha=weight), show.legend = FALSE,curvature=-0.5,data = SA)
