...
process_set <- function(n){
  ...
  pcdf <- data.frame(prcomp(data.matrix(afp[x,]))$rotation, classify)
  ggplot(pcdf, aes(x = PC1, y = PC2, color = classify)) +  geom_point()
  ...
  write.table(assign_df, file = paste0("../output/firstpass_assignments_mix",n,".tsv"),
  ...
  pdf(paste0("../output/Pearson_Mix_Homoplasmic",n,"paper.pdf"), width=5, height=1.2)
  ...
  png(paste0("../output/Pearson_Mix_Homoplasmic",n,"paper.png"), width=20, height=4.8, res = 1000, units =  "in")
  hm <- Heatmap(data.matrix(afp[vv,as.character(assign_df$barcode)]), 
                col=as.character(jdb_palette("solar_rojos",type="continuous")),
                show_row_names = TRUE, 
                cluster_columns = FALSE,
