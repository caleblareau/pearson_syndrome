...
save(important_df, umap, lsiPeaks, means_lsi, lsiReference, file = "12september2019_projection_5k_CLL.rda")

p1 <- ggplot(important_df %>% filter(batch == "base"), aes(x = umap1, y = umap2, color = cell_cluster)) +
  geom_point_rast(size = 0.05, raster.dpi = 500) +
  pretty_plot(fontsize = 8) + L_border() + labs(x = "UMAP1", y = "UMAP2") +
  theme(legend.position = "none")

cowplot::ggsave(p1, file = "plots/base_umap_embedding.pdf", width = 2, height = 2)
